# Inkbit Illustration · 잉크 점묘 일러스트

[简体中文](README.md) · [繁體中文](README.zh-TW.md) · [English](README.en.md) · [日本語](README.ja.md) · [한국어](README.ko.md)

Inkbit 흑백 잉크 일러스트를 만들거나 수정합니다. 윤곽, 점묘, 짧은 해칭과 여백을 활용하는 대표 이미지와 삽화에 사용하며 일반 웹사이트 개편이나 정확한 UI·로고 재현에는 사용하지 않습니다.


## 선정된 예시

![Creative workspace](skills/inkbit-illustration/assets/examples/07-workspace.png)

![City cycling](skills/inkbit-illustration/assets/examples/08-city-cycling.png)

![Listening to music](skills/inkbit-illustration/assets/examples/09-music.png)

![Baking kitchen](skills/inkbit-illustration/assets/examples/10-baking.png)

![Lakeside camping](skills/inkbit-illustration/assets/examples/11-camping.png)

![Sailing harbor](skills/inkbit-illustration/assets/examples/12-harbor.png)

![Reading corner](skills/inkbit-illustration/assets/examples/13-reading-corner.png)

[Image prompts](skills/inkbit-illustration/references/selected-gallery.md) · MU Labs / Inkbit Illustration · CC BY 4.0


## 설치와 사용

`skills/inkbit-illustration` 폴더 전체를 호스트의 스킬 위치에 복사합니다. 하나만 설치하세요. 기본 언어는 중국어 간체이며 번체, 영어, 일본어, 한국어로도 요청할 수 있습니다.

```text
$inkbit-illustration을 사용해 여행 주제의 흑백 점묘 일러스트를 그려 주세요. 글자는 넣지 마세요.
```

[Skill](skills/inkbit-illustration/SKILL.ko.md) · [Gallery / 图集](skills/inkbit-illustration/references/gallery.md)

## 라이선스

재사용·수정할 때 MU Labs / Inkbit Illustration, 출처, CC BY 4.0 링크를 유지하고 변경 사항을 표시하세요.

[NOTICE](NOTICE.md) · [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) · [Source](https://github.com/mustundead/inkbit-illustration-skill)
